...
# Tratamento dos textos das colunas de Descrições: 'Seção', 'Dvisão', 'Grupo', 'Classe' e 'Subclasse' para minúsculo.
    df["Seção"] = df["Seção"].apply(lambda x: (x.lower()))
    df["Divisão"] = df["Divisão"].apply(lambda x: (x.lower()))
    df["Grupo"] = df["Grupo"].apply(lambda x: (x.lower()))
    df["Classe"] = df["Classe"].apply(lambda x: (x.lower()))
    df["Subclasse"] = df["Subclasse"].apply(lambda x: (x.lower()))

    # Tratamento dos textos das colunas de Descrições: 'Seção', 'Dvisão', 'Grupo', 'Classe' e 'Subclasse' para retirar acentos.
    df = df.applymap(lambda x: (x) if type(x) == str else x)
